...
function collectPageAndContainerTags(source) {
  return [...source.matchAll(/<(main|section|aside|article|header|footer)\b[^>]*>/gi)].map((match) => match[0]);
}
...
test("pages and containers keep unique identification, role, and title metadata", () => {
  ...
  for (const filePath of sourceFiles) {
    if (!fs.existsSync(filePath)) {
      findings.push(`${path.relative(projectRoot, filePath)} is missing from the page/container audit`);
      continue;
    }
    ...
    if (filePath.endsWith("input.css")) {
      const broadSelectors = ["main", "section", "aside", "article", "div", "header", "footer", "page", "container"]
        .filter((selector) => new RegExp(`(^|[\\s,{])${selector}(?=[\\s,{])`, "i").test(source));
      ...
      if (selectors.some((selector) => /^(page-|container-|content-|layout-|shell-|header-|footer-)/.test(selector))) {
    ...
    for (const tagString of collectPageAndContainerTags(source)) {
      ...
      if (!classes) {
        findings.push(`${relativePath} contains a ${tagName} without a dedicated styling class`);
      }

      if (["main", "article", "section", "aside"].includes(tagName) && !/main|region|complementary|article|tabpanel/i.test(role)) {
        findings.push(`${relativePath} contains a page or container section without a meaningful role`);
      }

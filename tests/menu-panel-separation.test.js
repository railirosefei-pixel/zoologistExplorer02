...
test("menus and panels keep unique identification, role, and title metadata", () => {
  ...
  for (const filePath of sourceFiles) {
    if (!fs.existsSync(filePath)) {
      findings.push(`${path.relative(projectRoot, filePath)} is missing from the menu/panel audit`);
      continue;
    }
    ...
    if (filePath.endsWith("input.css")) {
      const broadSelectors = ["nav", "section", "aside", "main", "menu", "panel"]
    ...
    for (const tagString of collectStructuralTags(source)) {
      ...
      if (tagName === "nav" && !/navigation/i.test(role)) {
        findings.push(`${relativePath} contains a navigation menu without a navigation role`);
      }

  ...
  for (const filePath of vueFiles) {
    ...
    for (const styleBlock of styleBlocks) {
      ...
      if (!allowedSelectorPrefixes) {
        findings.push(`${relativePath} owns component CSS without an explicit ownership rule`);
        continue;
      }
      ...
      if (
        selectors.some(
          (selector) => !allowedSelectorPrefixes.some((prefix) => selector.startsWith(prefix))
        )
      ) {
